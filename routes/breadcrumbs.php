...
Breadcrumbs::for('competitions.show', function (
...
Breadcrumbs::for('competitions.parse', function (
    BreadcrumbTrail $trail,
    Competition $competition,
    Media $media,
) {
...
Breadcrumbs::for('competitions.event', function (
    BreadcrumbTrail $trail,
    Competition $competition,
    $event,
    $gender,
...
Breadcrumbs::for('events.show', function (
    BreadcrumbTrail $trail,
    ...
    $trail->push(
        $event->name . ' ' . strtolower($gender->description),
        route('events.show', [$event, $gender->getSlug()]),
    );
});

Breadcrumbs::for('athletes.show', function (
    BreadcrumbTrail $trail,
    Athlete $athlete,
) {
    $trail->parent('home');
...
Breadcrumbs::for('athletes.event', function (
    BreadcrumbTrail $trail,
    Athlete $athlete,
    $event,
) {
...
Breadcrumbs::for('teams.event', function (
    BreadcrumbTrail $trail,
    Team $team,
    $event,
    $gender,
    ...
    $trail->push(
        $event->name,
        route('teams.event', compact('team', 'event', 'gender')),
    );
});
