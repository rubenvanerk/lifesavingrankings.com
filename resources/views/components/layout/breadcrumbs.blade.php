@unless ($breadcrumbs->isEmpty())
    <nav class="flex py-4" aria-label="Breadcrumb">
        <ol role="list" class="flex items-center md:space-x-4">
                ...
                @else
                    <li class="hidden sm:block">
                        <div class="flex items-center">
                            <x-heroicon-s-chevron-right class="shrink-0 h-5 w-5 mt-0.5 text-white"/>
                            @if ($loop->last)
                                <span class="ml-4 text-sm font-semibold text-amber-300">{{ $breadcrumb->title }}</span>
                            @else
            ...
            <li class="sm:hidden">
                <a href="{{ $parent->url }}"
                   class="text-sm font-medium hover:text-amber-300 text-gray-100 flex items-center space-x-1">
                    <x-heroicon-s-chevron-left class="shrink-0 h-4 w-4 mt-0.5"/>
                    <span>
                        {{ $parent->title }}
                    </span>
                </a>
